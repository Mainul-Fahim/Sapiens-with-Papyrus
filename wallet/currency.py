...
class ConcurrentDepositTestCase(WalletTestCase):
    ...
    def test_deposit(self):
        ...
        def deposit_thread():
            with transaction.atomic():
                ...
                """Simulating a long transaction so that
                 when the other thread comes in, this
                 thread still holds the lock."""
                time.sleep(1)

        """Running the two threads to simulate two
        ...
        """Retrieving a new wallet object since what we
         currently have is now stale."""
        wallet = Wallet.objects.get(pk=self.wallet.id)

        """ Checking that the current_balance wasn't
        ...
        """Checking that we have two transactions. Each
    ...
    def test_withdraw(self):
        ...
        """Checking that the number of transactions
    ...
    def test_multiple_withdraw_insufficient_balance(self):
        """Testing two concurrent withdraw
        ...
        """ Asserting that the only the first transaction was
...
class ConcurrentTransferTestCase(WalletTestCase):
    ...
    def test_transfer(self):
        """Simulating concurrent transfer to a
        single wallet."""
        INITIAL_BALANCE = 200
        self._create_initial_balance(INITIAL_BALANCE)
        TRANSFER = 100
        ...
        def transfer_thread():
            with transaction.atomic():
                ...
                wallet.transfer(wallet2, TRANSFER)
                 ...
                 """Simulate a long transaction so that
                 when the other thread comes in, this
                 thread still holds the lock."""
                time.sleep(1)
        ...
        """ Asserting that all transfers were made and that
